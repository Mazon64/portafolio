import { ArrowUpRightIcon, BriefcaseBusinessIcon, BracesIcon, FilesIcon, FolderKanbanIcon, GraduationCapIcon, UserRoundIcon } from "lucide-react";
  ...
  return (
    <div>
      ...
      <div className="mt-14 grid gap-5 md:grid-cols-2">
        ...
        <Link href={`/admin/${lang}/documents`} className="group rounded-3xl border border-border bg-card p-7 text-card-foreground transition-transform hover:-translate-y-1">
          <div className="flex items-start justify-between gap-4"><FilesIcon className="size-7" aria-hidden="true" /><ArrowUpRightIcon className="size-5 transition-transform group-hover:translate-x-1 group-hover:-translate-y-1" aria-hidden="true" /></div>
          <h2 className="mt-12 text-2xl font-semibold tracking-[-0.03em]">{copy.documentsTitle}</h2>
          <p className="mt-3 leading-7 text-muted-foreground">{copy.documentsDescription}</p>
          <p className="mt-6 font-mono text-xs tracking-[0.16em] uppercase">{copy.manage}</p>
        </Link>
      </div>
    </div>
