...
export default async function ConversationsPage({ params, searchParams }: { params: Promise<{ lang: string }>; searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  catch (error) { if ((error as { code?: string }).code === "P2021") return <p>{copy.schemaPending}</p>; throw error; }
  ...
  return <div>
    ...
    {!data.conversations.length && <p className="rounded-2xl border border-dashed border-border p-8 text-muted-foreground">{copy.empty}</p>}
