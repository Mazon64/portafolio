...
`AiContext`, `JobApplication` y `DocumentArtifact` forman un subsistema privado. El contexto actual se actualiza en su misma fila; cada solicitud guardada conserva la vacante original y produce artefactos `ATS_CV` y `COVER_LETTER`. Los artefactos registran locale, modelo, hash de fuentes, estado y contenido JSONB validado, sin contadores internos. `AiContext` conserva el nombre físico de la tabla mediante un mapping; la migración final impone un único contexto actual. La transición de esquema se detalla en [document-schema-transition.md](document-schema-transition.md). El CMS permite filtrar y eliminar artefactos; cuando una solicitud pierde su último artefacto, también elimina la fila huérfana. No se almacena HTML generado.
...
`ProjectIntegration` vincula un repositorio público por ID estable. El worker descubre metadatos, documentación, manifiestos, una muestra de código/pruebas, actividad de GitHub e imágenes. Gemini genera y reevalúa hitos fundamentados; el servicio valida citas textuales, conserva IDs y guarda estado, justificación ES/EN, extractos y hashes junto al snapshot en la columna JSONB `ProjectKnowledge.narrative`. No hay archivo de hitos requerido ni edición/publicación manual. Las imágenes se descubren en enlaces Markdown y carpetas habituales u opcionales por propósito; los binarios permanecen en GitHub. El avance cuenta objetivos completados con pesos iguales, incluyendo los no verificados en el denominador; no representa esfuerzo ni terminación total. Las comprobaciones de lease y fuentes protegen la sustitución atómica de la publicación anterior. Los snapshots legacy se reevalúan automáticamente en la próxima sincronización, sin DDL adicional.

Gemini redacta campos estructurados ES/EN y analiza imágenes reales normalizadas por Sharp; las clasifica por carpetas `interface`, `features`, `diagrams` y `results` y describe solo contenido observable. Reutiliza el análisis visual por hash/modelo/política. `gemini-embedding-001` produce vectores normalizados de 768 dimensiones; `ProjectKnowledgeChunk` los almacena en `extensions.vector(768)`. RAG recupera hasta seis fragmentos públicos, incluidas observaciones visuales identificadas como tales, valida citas y responde con referencias o abstención. Las cuotas no guardan conversaciones. La recuperación automática combina el webhook, el cron diario y un scheduler de quince minutos en GitHub Actions. Véase [project-integration.md](project-integration.md).
