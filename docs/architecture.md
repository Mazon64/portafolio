...
`AiContextVersion`, `JobApplication` y `DocumentArtifact` forman un subsistema privado. Cada guardado de contexto crea una fila nueva; cada solicitud guardada conserva la vacante original y produce artefactos `ATS_CV` y `COVER_LETTER`. Los artefactos registran locale, una secuencia interna no presentada al usuario, modelo, hash de fuentes, estado y contenido JSONB validado. El CMS permite filtrar y eliminar artefactos; cuando una solicitud pierde su último artefacto, también elimina la fila huérfana. No se almacena HTML generado.
