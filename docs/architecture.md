...
`AiContextVersion`, `JobApplication` y `DocumentArtifact` forman un subsistema privado append-only. Cada guardado de contexto crea una fila nueva; cada solicitud guardada conserva la vacante original y produce artefactos `ATS_CV` y `COVER_LETTER`; no existen acciones de actualización o borrado para ese historial. Los artefactos registran locale, versión, modelo, hash de fuentes, estado y contenido JSONB validado. No se almacena HTML generado.
